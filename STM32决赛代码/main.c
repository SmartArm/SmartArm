...
int *GetIntData();
void updateData(int *data);//用以更新控制数据的函数
...
int theta1=0,theta2=0,theta3=0;//theta1是底座舵机角度,theta2是大臂舵机角度,theta3是小臂舵机角度
...
int gotHand=0,x=0,y=0,z=0,pitch=90,roll=90,yaw=0,pinch=100, circleGesture=0,tapGesture=0,speed=0,
    normalPause=0,autoTrack=0,camX=0,camY=0;//gotHand代表当前用户的手是否在识别区里
 uint32_t TimeNow=0,LastTime=0;//分别用以记录现在的时间和上一次进行PID计算的时间，单位为ms
 /*********************************以下是自动追踪PID算法的函数************************************************/
typedef struct PID
{
       ...
       //uint32_t Time1=0,Time2=0;
        ...
        while(1){
         ...
         if(ready==1){
           ...
           {
             TimeNow=getTime();
             if(TimeNow-LastTime>=50)//每50MS进行一次PID计算
             {
            /*  SetTarget(Xptr,camX);//设置X坐标目标
              SetTarget(Yptr,camY);//设置Y坐标目标
              int xInc=0,yInc=0;
              xInc=IncPIDCalc(x,Xptr);
              yInc=IncPIDCalc(y,Yptr);//计算增量
              x+=xInc;
              y+=yInc;//更新输出
              //以下是执行代码
...
void updateData(int *data){
  ...
  normalPause=*(data+11);//是否进入暂停控制状态
  autoTrack=*(data+12);//是否进入自动追踪状态
  camX=*(data+13);//追踪对象的X坐标
  camY=*(data+14);//追踪对象的Y坐标
}
...
void ManualControl(void)
{
        ...
        //  {
            ...
            if(theta3<-90) theta3=90;
            runServoTo(servo_ptr,roll);//手腕翻滚
            runServoTo(servo_ptr+1,pitch);//手腕俯仰
